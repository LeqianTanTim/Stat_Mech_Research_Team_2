...
# Part 1. File Input
particle_lst, dimension = F24_MD_code_Tim_Tan.load_particles_from_csv('MD_test.csv')
...
# Define a threshold for outliers
threshold = 400 # For temperature 
# Filter energy_avg and steps
filtered_steps = steps[temeperatures < threshold]
filtered_temp_avg = temeperatures[temeperatures < threshold]
# Plot the filtered data
plt.figure(figsize=(8, 6))
plt.plot(filtered_steps, filtered_temp_avg, label='Filtered Temperature', linewidth=2)
plt.xlabel('Steps', fontsize=12)
plt.ylabel('Temp', fontsize=12)
plt.title('Filtered Temperature vs. Number of Steps', fontsize=14)
plt.grid(True, linestyle='--', alpha=0.6)
plt.legend(fontsize=12)
plt.show()