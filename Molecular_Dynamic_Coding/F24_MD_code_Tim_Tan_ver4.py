...
def compute_forces(particle_lst, potential_energies, box_size, dimensions):
    ...
    for i in range(num_particles - 1):
      ...
      if not particle_lst[i].fixed_or_not:
        ...
        new_acc = current_acc * 0.25 * 1e20 * box_size+ np.sum(forces, axis=0)/particle_lst[i].mass # convert the acceleration to m/s^2 before adding F/m = a
...
def scale_velocity(particle_lst, scale, time_step):
    for particle in particle_lst:
        if not particle.fixed_or_not:
            old_vel = particle.get_vel()
            new_vel = scale * old_vel + particle.get_acc() * 0.5 * time_step
...
def complete_force_update(particle_lst, time_step):
    for particle in particle_lst:
        if not particle.fixed_or_not:
            ...
            particle.update_vel(current_vel + time_step * particle.get_acc() * 0.5)
    return particle_lst
...
def plot(steps, data_to_plot, title, upper_threshold=1e10, lower_threshold=0.0):
    filter_plots = steps[(data_to_plot < upper_threshold) & (data_to_plot > lower_threshold)]
    filter_data = data_to_plot[(data_to_plot < upper_threshold) & (data_to_plot > lower_threshold)]
    plt.figure(figsize=(8, 6))
    plt.plot(filter_plots, filter_data, label=f'{title}', linewidth=2)
    plt.xlabel('Steps', fontsize=12)
    plt.ylabel(f'{title}', fontsize=12)
    plt.title(f'{title} vs. Step', fontsize=14)
